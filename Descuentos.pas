...
begin
    ...
    if(txtDiaIni.Text = '') or (txtMesIni.Text = '') or (txtAnioIni.Text = '') then begin
        ...
        if(dFecha1 > dFecha2) then begin
            ...
            Application.MessageBox('Introduce un artículo válido','Error',[smbOK],smsCritical);
            txtCodigo.SetFocus;
            Result := False;
        end
        else if (rdgTipoDesc.ItemIndex = 1) and (cmbCategorias.ItemIndex < 0) then begin
            Application.MessageBox('Selecciona una categoría','Error',[smbOK],smsCritical);
...
begin
    with dmDatos.qryListados do begin
        ...
        dmDatos.cdsCliente.FieldByName('articulo').DisplayLabel := 'Artículo';
...
begin
    pgeGeneral.ActivePage := tabDatos;
    if(Application.MessageBox('Se eliminará el descuento seleccionado','Eliminar',[smbOK]+[smbCancel],smsWarning) = smbOK) then begin
        with dmDatos.qryModifica do begin
            Close;
            SQL.Clear;
