...
begin
  {$IFDEF MSWINDOWS}
      ...
      begin
        Application.MessageBox('El sistema Ventas 1.0.1 ya se está ejecutando','Información',[smbOk],smsInformation);
        Halt(0)
      end;

      if (Screen.Width < RESOLUCION_HORIZONTAL) or (Screen.Height < RESOLUCION_VERTICAL) then
      begin
        StrPCopy(szResolucionMinima,'El sistema Ventas 1.0.1 se visualiza mejor con la resolución mínima de ' + IntToStr(RESOLUCION_HORIZONTAL) + 'x' + IntToStr(RESOLUCION_VERTICAL) + ' pixeles');
        Application.MessageBox(szResolucionMinima, 'Información', [smbOk],smsWarning);
      end;
  {$ENDIF}
